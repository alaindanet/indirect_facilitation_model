...
drhonp <- function(rho, parms, z = 4) {
  with(parms,
    (rho[4] - rho[2] - rho[1]) * (del * rho[5] + (1 - del) * (z-1)/z * (rho[5]
    - rho[3] - rho[1])/(1- rho[5] - rho[4]))*(b- cn* rho[5] - cpn*
	  rho[4] ) + (rho[5] - rho[3] - rho[1]) * (del * rho[4] + (1-del) *
      (z-1)/z * (rho[4] - rho[2] - rho[1])/(1- rho[5] - rho[4])) * (b- cp*
  rho[4] - cnp* rho[5] - g*( 1 - 1/z*n - ( z-1 )/z * (rho[5] - rho[3] -
	  rho[1])/(1- rho[5] - rho[4])*n) ) - 2 * rho[1] * m)
}
