...
parallel_param <- list(b1 = seq(0.9,0.4,-0.1),
                       g = seq(0.0,0.9,0.05),
                       n = seq(0,1,1),
                       c_21 = seq(0.1,0.9,0.05),
                       c_12 = 0.1
...
foreach(iteration = iterations$ID, .combine = rbind) %dopar% { 
  ...
  while(nrep+1 <= min_replicates & j+1 <= max_tries) {
    ...
    while(stability_total > 0.0001 | stability_nurse > 0.05 | stability_protege > 0.05 & i <= t_max/delta & result$rho_plus[i] > 0) {
      ...
      recolonisation1 <- with(parms_temp, (del1*rho_nurse+(1-del1)*Q_plus1)*(b1-c_1*Q_plus1-c_21*Q_plus2-g*Q_plus2*p)*delta)
      recolonisation2 <- with(parms_temp, (del2*rho_protege+(1-del2)*Q_plus2)*(b2-c_2*Q_plus2-c_12*Q_plus1-g*(1-Q_plus1*n))*delta)
  ...
  result$out <- data.frame(
    ID = iteration,
    del1 = parms_temp$del1,
    del2 = parms_temp$del2,
    g = parms_temp$g, 
    b = parms_temp$b1, 
    m = parms_temp$m1,
    com = parms_temp$com,
  ...
  save(result, file = paste("result_coex_scenario1", iteration,".rdata", sep = ""))
...
write.csv(output, file = paste("output_coex_scenario1.csv", sep = "") )# }}}
