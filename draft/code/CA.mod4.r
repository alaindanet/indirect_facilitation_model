...
# I just want to keep the iterations which is interesting for me:
...
foreach(iteration = iterations$ID, .combine = rbind) %dopar% { 
  ...
  save(result, file = paste("result.coex.27.05", iteration,".rdata", sep = ""))
...
write.csv(output, file = paste("output.coex.27.05.csv", sep = "") )# }}}
