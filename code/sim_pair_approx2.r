...
#' Author: A Danet
#' Model: Remplacement of global competition by local one.
#'###################################################################################'

# TODO: Modify competition terms.
rm(list=ls())
...
# Parallel parameters setting
	 modelparms <- list(del = seq(0.1, 0.9, 0.2),
			    b   = seq(0.8, 0.4, -0.1),
...
dir.create("pairs_mod2")
setwd(paste(getwd(),"/pairs_mod2",sep=""))
