...
output <- run_scenarii_gradient(
  ...
  param = c(protection_type = list("first_protect"), gamma1 = 0.1, g = .2, u = 5),
...
rm(output)
save(scenar_avg, file = "f_b_effect_pa_avg.RData")
...
output <- run_scenarii_gradient(
  gradient = gradient,
  model_spec = "two_facilitation_model",
  param = c(protection_type = list("first_protect"), gamma1 = 0.1, g = .2),
  scenarii = init_scenarii(type = "together", ini_cover = .8),
  set_tail = 10
  )
...
scenar_avg <- avg_runs(output, cut_row = 10)
rm(output)
save(scenar_avg, file = "u_b_effect_pa_avg.RData")
