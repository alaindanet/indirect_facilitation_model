...
plot_grid(state_u, clust_u, qpn_u, labels = "AUTO", nrow = 1)
```
...
interp_data <- interp_contour(clust_scenar, x = del, y = f, z = clustering, nb_pts = 100, duplic = clust_type)

contour_c_f <- cnp_cveg_f +
  draw_contour(interp_data, x = x, y = y, z = clustering, colour = "black", breaks = c(3, 3.5, 4, 4.5)) 

c_del_f <- direct.label(contour_c_f, list("bottom.pieces", colour = "black"))
...
rho_qpn_f <- plot_diagram(clust_np, param = c(x = "del", y = "f"), fill =
  "qpn") + 
  theme_alain() +
  xylabs(x = "del", y = "f") +
  scale_fill_temperature(name = "Average # of \n protegee neighbors \n for nurse cells", limits = c(0,2)) 
...
interp_qpn_f <- interp_contour(clust_np, x = del, y = f, z = qpn, nb_pts = 50)

contour_qpn_f <- rho_qpn_f +
  draw_contour(interp_qpn_f, x = del, y = f, z = qpn, nb_pts = 50, colour = "black", breaks = c(.25, .75)) 
...
```{r}
plot_grid(state_f, cnp_cveg_f, p_qpn_f, labels = "AUTO", nrow = 1)
```
...
clustering_plot <- plot_grid(
  state_u, clust_u, qpn_u, state_f, cnp_cveg_f, rho_qpn_f,
  labels = "AUTO",
  nrow = 2,
...
scenar_avg$run %<>% gather(rho, value, N, P) %>% 
  ...
  ungroup() %>% select(-ind) %>%
  mutate(
    ca = ifelse(ca < .001, 0, ca),
    pa = ifelse(pa < .001, 0, pa),
