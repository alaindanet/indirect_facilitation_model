...
fig3 <- plot_bifurcation(filter(u0_5, g %in% g_val, u %in% u_val)) +
...
save_plot("~/Documents/thesis/papers/indirect_facilitation_dryland_stability/fig/bifurcation.pdf",
  fig3,
  base_width = 2.5,
  base_height = 4.3,
  ncol = 2)
...
filter(states$run, u %in%  5.0, del == 0.23)
...
qpn_u <- plot_diagram(clust_np, param = c(x = "del", y = "u"), fill =
  ...
  # limits arg 
  scale_fill_gradientn(colors = c("white", temp_cols("yellow", "orange", "red")), limits = c(0,2))
```
...
rho_qpn_f <- plot_diagram(clust_np, param = c(x = "del", y = "f"), fill =
  ...
  theme_alain() +
  xylabs(x = "del", y = "f") +
  scale_fill_temperature(name = "Average \n # of nurse neighbors \n for a protegee cell", limits = c(0,2))

```

```{r}
plot_grid(state_f, cnp_cveg_f, rho_qpn_f, labels = "AUTO", nrow = 1)
```

# Clustering plot  

```{r clustering plot}
clustering_plot <- plot_grid(state_u, clust_u, qpn_u, state_f, cnp_cveg_f, rho_qpn_f, labels = "AUTO", nrow = 2)
clustering_plot
