...
margin_set_up <- margin(t = 2, r = 0, b = 2, l = 2, unit = "pt")
...
left_panel <- plot_bifurcation(protegee_only, P) +
    ggplot2::scale_colour_manual(
      name = "Species",
      values = c(N = "#BBCC33", P = "#99DDFF"),
      labels = species_labeller()
    ...
    ggtitle("Protegee only")

legend_b <- cowplot::get_legend(fig3 + theme(legend.position="bottom"))

top_row <- cowplot::plot_grid(left_panel, fig3, labels = "AUTO", rel_widths = c(.55, 1))
...
clust_u <- plot_diagram(clust_scenar, param = c(x = "del", y = "u"), fill =
  "clustering") + 
  theme_alain() +
  xylabs(x = "del", y = "u") +
...
interp_u <- interp_contour(clust_scenar, x = del, y = u, z = clustering, nb_pts = 30, duplic = clust_type)

brks_clustering <- c(3, 3.5, 4, 4.1, 4.2, 4.5)
contour_u <- clust_u +
  draw_contour(interp_u, x = x, y = y, z = clustering, colour = "black", breaks = brks_clustering) 

c_del_u <- direct.label(contour_u, list("bottom.pieces", colour = "black"))
```
...
interp_qpn_u <- interp_contour(clust_np, x = del, y = u, z = qpn, nb_pts = 30)
...
plot_grid(state_u, clust_u, p_qpn_u, labels = "AUTO", nrow = 1)
```
...
cnp_cveg_f <- plot_diagram(clust_scenar, param = c(x = "del", y = "f"), fill =
  "clustering") + 
  theme_alain() +
  xylabs(x = "del", y = "f") +
  scale_fill_temperature(name = "Clustering", limits = c(2, 4.5)) +
...
interp_f <- interp_contour(clust_scenar, x = del, y = f, z = clustering, nb_pts = 30, duplic = clust_type)

contour_c_f <- cnp_cveg_f +
  draw_contour(interp_f, x = x, y = y, z = clustering, colour = "black", breaks = brks_clustering) 

c_del_f <- direct.label(contour_c_f, list("bottom.pieces",
      box.color = NA, fill = "transparent", colour = "black"))
...
interp_qpn_f <- interp_contour(clust_np, x = del, y = f, z = qpn, nb_pts = 30)
...
```{r}
plot_grid(state_f, c_del_f, p_qpn_f, labels = "AUTO", nrow = 1)
```
...
clustering_plot <- plot_grid(
  state_u, clust_u, p_qpn_u, state_f, cnp_cveg_f, p_qpn_f,
  labels = "AUTO",
  nrow = 2,
  rel_widths = c(1,1.5,1,1,1.5,1)
  )
clustering_plot
